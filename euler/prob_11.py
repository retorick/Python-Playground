...
def calc_from_point(x, y):
    _horiz = reduce(lambda x, y: x*y, grid[x][y:y+4])
    prods.append(_horiz)
    print 'Row: ', grid[x][y:y+4], _horiz

    _vertnums = []
    for _i in xrange(4):
        _vertnums.append(grid[x+_i][y])
    _vert = reduce(lambda x, y: x*y, _vertnums)
    prods.append(_vert)
    print 'Col: ', _vertnums, _vert

    _diagnums = []
    _diagnums2 = []
    for _i in xrange(4):
        _diagnums.append(grid[x+_i][y+_i])
        _diagnums2.append(grid[x+_i][y+3-_i])
    _diag = reduce(lambda x, y: x*y, _diagnums)
    _diag2 = reduce(lambda x, y: x*y, _diagnums2)
    print 'Diag: ', _diagnums, _diag
    print 'Diag2: ', _diagnums2, _diag2
    prods.append(_diag)
    prods.append(_diag2)
...
print "Maximum product: %s" % max(prods)
